...
def orientation_and_position_cost(images, states, pad, car_size=(6.4, 14.3), unnormalize=False, s_mean=None, s_std=None,
    ...
    position_cost = -torch.log(torch.mean(torch.mean(torch.max(torch.stack([v*100, torch.ones_like(v)*position_threshold],
...
def parse_command_line(parser=None):
    ...
    parser.add_argument('-learned_cost', type=str, default='')
