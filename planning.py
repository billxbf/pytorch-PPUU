...
# was trained on
def estimate_uncertainty_stats(model, dataloader, n_batches=100, npred=200, pad=1, offroad_range=1.0):
...
def plan_actions_backprop(model, input_images, input_states, car_sizes, npred=50, n_futures=5, normalize=True,
                          ...
                          save_opt_stats=True, nexec=1, lambda_l=0.0, lambda_o=0.0, pad=1, offroad_range=1.0):
    ...
    for i in range(bprop_niter):
        ...
        if u_reg > 0.0:
            ...
            _, _, _, _, _, _, uncertainty_loss = compute_uncertainty_batch(
                ...
                Z=Z.permute(1, 0, 2).clone(), detach=False, compute_total_loss=True, pad=pad, offroad_range=offroad_range
            )
        ...
        if model.opt.use_colored_lane is True:
            orientation_loss, position_loss, speed_loss = utils.orientation_and_position_cost(pred_images, pred_states.data,
                                                                                      ...
                                                                                      offroad_range=offroad_range,
                                                                                      opt=model.opt,
                                                                                      speed_max=model.stats['speed_max'] if hasattr(model.opt,'use_speed_map') and model.opt.use_speed_map else None)
...
def train_policy_net_mpur(model, inputs, targets, car_sizes, n_models=10, sampling_method='fp', lrt_z=0.1,
                          n_updates_z=10, infer_z=False, pad=1, offroad_range=1.0):
        ...
        for k in range(n_updates_z + 1):
            ...
            if k < n_updates_z + 1:
                _, _, _, _, _, _, total_u_loss = compute_uncertainty_batch(
                    ...
                    detach=False, Z=Z_adv.permute(1, 0, 2), compute_total_loss=True, pad=pad, offroad_range=offroad_range
                )
    ...
    _, _, _, _, _, _, total_u_loss = compute_uncertainty_batch(
        model, input_images, input_states, pred_actions, targets, car_sizes, npred=npred, n_models=n_models,
        detach=False, Z=Z.permute(1, 0, 2), compute_total_loss=True, pad=pad, offroad_range=offroad_range
    )
